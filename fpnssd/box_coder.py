...
class FPNSSDBoxCoder:
    ...
    def _get_anchor_boxes(self, input_size):
        ...
        if torch.__version__ == '0.4.0':
            fm_sizes = [(input_size/pow(2.,i+3)).float() for i in range(num_fms)]
            ...
            fm_sizes = [(input_size/pow(2.,i+3)).float().ceil().long() for i in range(num_fms)]
            boxes = []
            for i in range(num_fms):
                fm_size = fm_sizes[i]
                ...
                wh = anchor_wh[i].view(1,1,9,2).expand(fm_h,fm_w,9,2)
                box = torch.cat([((xy).float()-wh/2.).long(),((xy).float()+wh/2.).long()], 3)  # [x,y,x,y]
                boxes.append(box.view(-1,4))
        return torch.cat(boxes, 0).float()
